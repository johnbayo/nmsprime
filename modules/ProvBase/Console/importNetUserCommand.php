...
// d($this->qoss->where('ds_rate_max_help', 262144)->where('us_rate_max_help', 262144)->first());
	...
	private function add_modem($new_contract, $old_modem, $db_con)
	{
		...
		if ($modem->configfile_id == 0)
			\Log::error('No Configfile could be assigned to Modem '.($modem->id)." Old ModemID: $old_modem->Lfd");
		if (!$modem->qos_id)
		...
		foreach ($types as $filter => $fields)
		{
			...
			for ($i=1; $i <= $max_numbers; $i++)
			{
				...
				foreach ($fields as $col_name => $oid)
				{
					...
					if (!isset($hit[0]) || !$hit[0]) {
						\Log::notice("Missing $col_name for phonenumber $i. Discard number of MTA $mta->id.");
						break;
					}
